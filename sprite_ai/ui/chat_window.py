...
class ChatWindow(QtWidgets.QWidget):
    ...
    def __init__(self, event_manager: EventManager):
        ...
        self.event_manager.subscribe('ui.add_message', self.message_recived.emit)
    
    def eventFilter(self, obj, event: QEvent):
        modifiers = QtWidgets.QApplication.keyboardModifiers()
        if event.type() == QEvent.KeyPress and obj is self.ui.te_chatinput:
            if event.key() == Qt.Key_Return and self.ui.te_chatinput.hasFocus() and not modifiers == Qt.ShiftModifier:
    ...
    def add_message(self, message: dict):
        ...
        if sender == 'ai':
            ...
            raise ValueError(f'Invalid sender "{sender}", must be "ai" or "user"')
